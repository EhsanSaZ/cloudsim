...
//            createVmsInDatacenter(getDatacenterIdsList().get(0));
    ...
    protected void subm(){
        ...
        Vm vm2 = new Vm(vmid2, this.getId(), mips/2, pesNumber, ram, bw, size, vmm, new CloudletSchedulerTimeShared());
        vmlist2.add(vm2);
        submitVMsInDataCenterDynamic(vmlist2);
    }
    /**
...
//                }
                ...
                //check if all datacenters for all vms are queried then continue
                // else call create vm in Datacenter and return
...
//        getDatacenterRequestedIdsList().add(datacenterId);
    ...
    public void submitVMsInDataCenterDynamic(List<? extends Vm> list){
        getVmList().addAll(list);
        createVMsInDataCenterDynamic();
    }

    protected void createVMsInDataCenterDynamic(){
//        setDatacenterIdsList(CloudSim.getCloudResourceList());
        ...
        for (Vm vm : getVmList()) {
            if (!getVmsToDatacentersMap().containsKey(vm.getId())) {
                ...
                if (!getVmToDatacenterRequestedIdsList().containsKey(vm.getId())){
                    ...
                    sendNow(nextDataCenterId, 5000, vm);
                    requestedVms++;
                }else{
                    List <Integer> requestedList = getVmToDatacenterRequestedIdsList().get(vm.getId());
                    for (int nextDataCenterId : getDatacenterIdsList()) {
